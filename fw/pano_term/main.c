...
#define DIRTY_KEYBOARD  (1<<0)
#define DIRTY_FUNCTION  (1<<1)
#define DIRTY_BUTTONS   (1<<2)
#define DIRTY_PADDLES   (1<<3)
#define DIRTY_GAMEPADS  (1<<4)
#define DIRTY_RESERVED0 (1<<5)
#define DIRTY_RESERVED1 (1<<6)
#define DIRTY_RESERVED2 (1<<7)

uint32_t gDefaultFlashAdr = 0x900000;

NetPrintInternal_t gShellPrint;
NetPrintInternal_t gTermPrint;

// Port Binding Sockets
struct tcp_pcb *gShell_pcb;
struct tcp_pcb *gTerm_pcb;

// Active Connection Sockets
struct tcp_pcb *gShellCon;
struct tcp_pcb *gTermCon;

#define REG_WR(reg, wr_data)       *((volatile uint32_t *)(reg)) = (wr_data)
#define REG_RD(reg)                *((volatile uint32_t *)(reg))

int I2CWrite(uint8_t Bus, uint8_t Device, uint8_t Address, uint8_t Data);
int I2CRead(uint8_t Bus, uint8_t Device, uint8_t Address, uint8_t *pData);
int I2CReadBlock(uint8_t Bus, uint8_t Device, uint8_t Address, uint8_t Len, uint8_t *pData);
...
void TermInit(void);
err_t TermAccept(void *arg, struct tcp_pcb *newpcb, err_t err);
err_t TermRecv(void *arg, struct tcp_pcb *tpcb, struct pbuf *p, err_t err);
void TermError(void *arg, err_t err);
err_t TermPoll(void *arg, struct tcp_pcb *tpcb);
err_t TermSent(void *arg, struct tcp_pcb *tpcb, u16_t len);

void ShellInit(void);
err_t ShellAccept(void *arg, struct tcp_pcb *newpcb, err_t err);
err_t ShellRecv(void *arg, struct tcp_pcb *tpcb, struct pbuf *p, err_t err);
void ShellError(void *arg, err_t err);
err_t ShellPoll(void *arg, struct tcp_pcb *tpcb);
err_t ShellSent(void *arg, struct tcp_pcb *tpcb, u16_t len);

void ConPuts(char *String);
int ConPrintf(const char *Format, ...);
void NetPuts(char *String);
void NetWaitBufEmptyInternal(NetPrintInternal_t *p, struct tcp_pcb *tpcb, const char *funct,int Line);
#define NetWaitBufEmpty(p,tpcb) NetWaitBufEmptyInternal(p,tpcb,__FUNCTION__,__LINE__)

int NetDumpHex(void *Data,int Len,bool bWithAdr,int Adr);
int NetPrintFillPcb(NetPrintInternal_t *p, struct tcp_pcb *tpcb);
void NetSendPrompt(void);
...
uint32_t ParseDataBlock(int apply, int verbose);
void AddBoardInfo(Tag_t Tag,size_t Len,void *TagData);
int ButtonPressed(void);
void ReBoot(uint32_t SpiAdr);

const char gVerStr[] = "Pano Term vx.xx compiled " __DATE__ " " __TIME__ " ";
...
int GPIOStatusCmd(char *CmdLine);
int GPIODirCmd(char *CmdLine);
int GPIOSetCmd(char *CmdLine);
int GPIOClearCmd(char *CmdLine);
int AXIReadCmd(char *CmdLine);
int AXIWriteCmd(char *CmdLine);
int I2CStatusCmd(char *CmdLine);
int I2CWriteCmd(char *CmdLine);
int I2CReadCmd(char *CmdLine);
int InfoCmd(char *CmdLine);
int ExitCmd(char *CmdLine);
int CommitCmd(char *CmdLine);
int PowerCmd(char *CmdLine);
int ShowConfigCmd(char *CmdLine);

const CommandTable_t gCmdTable[] = {
   { "exit     ",  "Disconnect",NULL,ExitCmd},
   ...
   { "gpio     ",  "Display GPIO Register",NULL,GPIOStatusCmd},
   { "gset     ",  "<bitmask>",NULL,GPIOSetCmd},
   { "gclr     ",  "<bitmask>",NULL,GPIOClearCmd},
   { "gdir     ",  "<bitmask>",NULL,GPIODirCmd},
   { "awrite   ",  "<addr> <data>",NULL,AXIWriteCmd},
   { "aread    ",  "<addr>",NULL,AXIReadCmd},
   { "i2cstatus",  "<bus>",NULL,I2CStatusCmd},
   { "i2cwrite ",  "<bus> <dev> <addr> <data>",NULL,I2CWriteCmd},
   { "i2cread  ",  "<bus> <dev> <addr>",NULL,I2CReadCmd},
   { "commit   ",  "Save Defaults",NULL,CommitCmd},
   { "showcfg  ",  "Show Defaults",NULL,ShowConfigCmd},
...
void dvi_setup(uint8_t encoder) {
   int i;

   for(i = 0; i < (sizeof(DVI_SETUP)/sizeof(uint8_t)); i+=2) {
      I2CWrite(I2C_BUS_ENC, encoder, DVI_SETUP[i+0], DVI_SETUP[i+1]);
   }
}

void handle_input(uint8_t bus, uint8_t dev) {
   uint8_t Byte;
   uint8_t Buffer[2];
   uint8_t IODirty;
   uint8_t Wake = 0;

   if(I2CRead(bus, dev, 0x00, &IODirty) != RESULT_OK)
      return;

   // to receive it, grab the keycode and push it to the Apple.
   if(IODirty & DIRTY_KEYBOARD) {
      if(I2CRead(bus, dev, 0x08, &Byte) == RESULT_OK) {
      }
   }

   // Function Keys
   if(IODirty & DIRTY_FUNCTION) {
      if(I2CReadBlock(bus, dev, 0x06, 2, Buffer) == RESULT_OK) {
         if(Buffer[0] & (1<<0)) {
            // F1
         }
         if(Buffer[0] & (1<<1)) {
...
// Set LED GPIO's to output
   ...
   dvi_setup(I2C_ENC_DVI);
   dvi_setup(I2C_ENC_HDMI);
   ...
   if(gBoardID == BOARD_ID_DZ22_2) {
      ...
      REG_WR(GPIO_BASE + GPIO_DIRECTION,Temp);
      REG_WR(GPIO_BASE + GPIO_OUTPUT_SET, GPIO_BIT_PWR_STATUS2);

      // Dirty hack to make the DZ22 video work
      *((volatile uint32_t *)0x97000010) = 1050;
   }
   ...
   for(; ; ) {
      ...
      // Check for input every 15 milliseconds
      if((ThisLoop - LastLoop) > 15) {
         LastLoop = ThisLoop;
         ...
         if(gIsDZ22) {
            handle_input(I2C_BUS_HDMI, 0x2D);
         } else {
            handle_input(I2C_BUS_DVI, 0x2D);
         }
...
{
   err_t err;
   if((gTerm_pcb = tcp_new_ip_type(IPADDR_TYPE_ANY)) != NULL) {

     if((err = tcp_bind(gTerm_pcb,IP_ANY_TYPE,10023)) == ERR_OK) {
       gTerm_pcb = tcp_listen_with_backlog(gTerm_pcb,1);
       tcp_accept(gTerm_pcb,TermAccept);
     }
     else {
...
err_t TermAccept(void *arg, struct tcp_pcb *newpcb, err_t err)
{
   ...
   if(err == ERR_OK && newpcb != NULL) {
      if(gTermCon != NULL) {
         ELOG("gShellCon not NULL\n");
      }
      gTermCon = newpcb;
      tcp_arg(newpcb,NULL);
      tcp_recv(newpcb,TermRecv);
      tcp_err(newpcb,TermError);
      tcp_poll(newpcb,TermPoll,0);
      tcp_sent(newpcb,TermSent);
   // Reset everything for a new connection
      gTermPrint.BytesSent = gTermPrint.Len = gTermPrint.BytesQueued = 0;
      ret_err = ERR_OK;
   }
...
{
   err_t Err;
   NetPrintInternal_t *p = &gTermPrint;

   VLOG("tpcb: %p\n",tpcb);
   NetPrintFillPcb(p, tpcb);
   return ERR_OK;
}
...
{
   err_t err;
   if((gShell_pcb = tcp_new_ip_type(IPADDR_TYPE_ANY)) != NULL) {

     if((err = tcp_bind(gShell_pcb,IP_ANY_TYPE,23)) == ERR_OK) {
       gShell_pcb = tcp_listen_with_backlog(gShell_pcb,1);
       tcp_accept(gShell_pcb,ShellAccept);
     }
     else {
        ELOG("tcp_bind failed: %d\n",err);
     }
...
err_t ShellAccept(void *arg, struct tcp_pcb *newpcb, err_t err)
{
   ...
   if(err == ERR_OK && newpcb != NULL) {
      ...
      gShellCon = newpcb;
      tcp_arg(newpcb,NULL);
      tcp_recv(newpcb,ShellRecv);
      tcp_err(newpcb,ShellError);
      tcp_poll(newpcb,ShellPoll,0);
      tcp_sent(newpcb,ShellSent);
   // Reset everything for a new connection
      gShellPrint.BytesSent = gShellPrint.Len = gShellPrint.BytesQueued = 0;
      gInputReady = false;
      gSendWelcome = true;
      gRxCount = 0;
...
{
   err_t Err;
   NetPrintInternal_t *p = &gShellPrint;
...
int ConPrintf(const char *Format, ...)
{
   ...
   Len = vsnprintf(PrintBuf,sizeof(PrintBuf)-1,Format,Args);
   PrintBuf[Len] = 0;
   ConPuts(PrintBuf);
}
...
void NetWaitBufEmptyInternal(NetPrintInternal_t *p, struct tcp_pcb *tpcb, const char *funct,int Line)
{
   if(tpcb != NULL && p->Len != p->BytesSent) {
...
int NetPrintFillPcb(NetPrintInternal_t *p, struct tcp_pcb *tpcb)
{
   ...
   if(tpcb != NULL && (Byte2Write = p->Len - p->BytesQueued) > 0) {
   // Have data to send
...
int DumpCmd(char *CmdLine)
{
   ...
   do {
      ...
      while(gShellCon != NULL && BytesRead < Len) {
         Bytes2Read = Len;
   ...
   do {
      ...
      while(Adr < FlashSize) {
         if(bPrintStart) {
            ...
            NetWaitBufEmpty(&gShellPrint,gShellCon);
         }
         Adr += EraseSize;
         bEmpty = CheckEmpty(Adr,PageSize,EraseSize);
...
int RebootCmd(char *CmdLine)
{
   ...
   if(!*cp)
      Adr = gDefaultFlashAdr;
   else if((Ret = GetAdrAndLen(&cp,&Adr,NULL)) != RESULT_OK)
      return Ret;
   ...
   do {
      ...
      *cp++ = 0;
      ...
      if(Ret = GetAdrAndLen(&cp,&gTftp.FlashAdr,NULL)) {
         gTftp.FlashAdr = gDefaultFlashAdr;
         //break;
         Ret = RESULT_OK;
      }
   ...
   do {
      switch(pPIB->Type) {
         case INFO_TYPE_CFG_WR:
            switch(pPIB->Register) {
               case CFG_REG_BRD_ID:
                  gBoardID = htonl(pPIB->Value);
                  break;
               case CFG_REG_MAC_MSB:
                  gEtherMac[1] = (pPIB->Value & 0xff000000) >> 24;
                  gEtherMac[0] = (pPIB->Value & 0xff0000) >> 16;
                  break;
               case CFG_REG_MAC_LSB:
                  gEtherMac[5] = (pPIB->Value & 0xff000000) >> 24;
                  gEtherMac[4] = (pPIB->Value & 0xff0000) >> 16;
                  gEtherMac[3] = (pPIB->Value & 0xff00) >> 8;
                  gEtherMac[2] = (pPIB->Value & 0xff);
                  break;
               case CFG_REG_SERIAL0:
               case CFG_REG_SERIAL1:
               case CFG_REG_SERIAL2:
               case CFG_REG_SERIAL3:
                  SerialByte = ((pPIB->Register >> 6) & 0xC);
                  SerialTemp = pPIB->Value;
                  gPanoSerial[SerialByte++] = (SerialTemp & 0xff000000) >> 24;
                  gPanoSerial[SerialByte++] = (SerialTemp & 0xff0000) >> 16;
                  gPanoSerial[SerialByte++] = (SerialTemp & 0xff00) >> 8;
...
// <32 bit tag> <32 bit length> <variable length data>...<tag><len><data>...
      ...
      // Skip to the next tag
         ...
         switch(Tag) {
            case TAG_TFTP_SRV:
               if(verbose)
                  NetPrintf("TFTP Server: %d.%d.%d.%d\r\n", (*((uint32_t *) pValue)) & 0xff, ((*((uint32_t *) pValue)) & 0xff00) >> 8, ((*((uint32_t *) pValue)) & 0xff0000) >> 16, ((*((uint32_t *) pValue)) & 0xff000000) >> 24);
               if(apply)
...
// read Pano data block into gTemp, return flash adr of first free word
uint32_t CheckDataTag(Tag_t cTag, size_t cLen, void *cTagData, void *cDefaultData)
{
   uint32_t DataPageOffset;
   uint32_t *pU32 = (uint32_t *) gTemp;
   uint32_t Tag;
   ...
   // If the data to be saved matches the default, and there is no entry in the config sector,
   // don't bother saving the default data to the config sector.  If there is no default, save.
   if(cDefaultData != NULL)
      TagDirty = (memcmp(cTagData, cDefaultData, cLen) != 0);
...
// <32 bit tag> <32 bit length> <variable length data>...<tag><len><data>...
      ...
      // Skip to the next tag
         ...
         if(Tag == cTag) {
            if(DataLen != cLen) {
               TagDirty = 1;
            } else if(memcmp(pValue, cTagData, cLen)) {
               TagDirty = 1;
            } else {
               TagDirty = 0;
            }
...
void AddBoardInfo(Tag_t Tag,size_t Len,void *TagData)
{
   ...
   uint32_t WriteOffset = ParseDataBlock(0, 0);
   uint32_t BufOffset;
   ...
   if(WriteOffset != 0) {
      ...
      // The new data fits, write it
         ...
         *pU32++ = (uint32_t) Len;
         ...
         ParseDataBlock(1, 0); // reload with updated data
      }
   }
}

void UpdateDataTag(Tag_t Tag, size_t Len, void *TagData, void *DefaultData) {
   if(CheckDataTag(Tag, Len, TagData, DefaultData)) {
      AddBoardInfo(Tag, Len, TagData);
   }
}

void UpdateDataTag32(Tag_t Tag, uint32_t TagData, uint32_t DefaultData) {
   uint32_t tData = TagData;
   uint32_t tDefault = DefaultData;
   UpdateDataTag(Tag, sizeof(uint32_t), &tData, &tDefault);
}

void UpdateDataTag32NoDefault(Tag_t Tag, uint32_t TagData) {
   uint32_t tData = TagData;
   UpdateDataTag(Tag, sizeof(uint32_t), &tData, NULL);
}
...
int CommitCmd(char *CmdLine) {
   ...
   UpdateDataTag32NoDefault(TAG_TFTP_SRV, gTftp.ServerIP.addr);
...
int I2CWrite(uint8_t Bus, uint8_t Device, uint8_t Address, uint8_t Data)
{
   ...
   while((Timeout > 0) && (REG_RD(I2C_BASE + I2C_CMD_STATUS) & I2C_STATUS_BUSY)) {
      Timeout--;
   }
   if(Timeout == 0) goto busy_timeout;

   // Fill the registers
   REG_WR((I2C_BASE + I2C_BUS),     Bus);
   REG_WR((I2C_BASE + I2C_DEVICE),  Device);
   REG_WR((I2C_BASE + I2C_ADDRESS), Address);
   REG_WR((I2C_BASE + I2C_DATA),    Data);
   ...
   REG_WR((I2C_BASE + I2C_CMD_STATUS), I2C_CMD_WRITE);
   REG_WR((I2C_BASE + I2C_CMD_STATUS), I2C_CMD_WRITE | I2C_CMD_ENABLE);
...
int I2CRead(uint8_t Bus, uint8_t Device, uint8_t Address, uint8_t *pData)
{
   ...
   // Fire the transaction
   REG_WR((I2C_BASE + I2C_CMD_STATUS), I2C_CMD_READ);
   REG_WR((I2C_BASE + I2C_CMD_STATUS), I2C_CMD_READ | I2C_CMD_ENABLE);
   ...
   if(REG_RD(I2C_BASE + I2C_CMD_STATUS) & I2C_STATUS_NACK) {
      LOG("\r\nI2C: NACK\r\n");
      Ret = RESULT_ERR;

   // Record the recieved data
   } else {
      *pData = REG_RD((I2C_BASE + I2C_DATA));
   }
...
int I2CStatusCmd(char *CmdLine)
{
   ...
   if(*CmdLine) {
      if(ConvertValue(&CmdLine,&Bus)) {
         Ret = RESULT_BAD_ARG;
      }
      else {
         NetPrintf("\r\nI2C0: %08X\r\n", REG_RD(I2C_BASE + I2C_CMD_STATUS));
      }
   }
   else {
      Ret = RESULT_USAGE;
...
int I2CWriteCmd(char *CmdLine)
{
   ...
   if(*CmdLine) {
      ...
      else if(ConvertValue(&CmdLine,&DevAddr)) {
         Ret = RESULT_BAD_ARG;
      }
      else if(ConvertValue(&CmdLine,&RegAddr)) {
         Ret = RESULT_BAD_ARG;
      }
      else if(ConvertValue(&CmdLine,&RegData)) {
         Ret = RESULT_BAD_ARG;
      }
      else {
         Ret = I2CWrite(Bus, DevAddr, RegAddr, RegData);
         if(Ret == RESULT_OK)
            NetPrintf("\r\nI2C: Success\r\n" );
         else
           NetPrintf("\r\nI2C: Bus Error\r\n");
...
int I2CReadCmd(char *CmdLine)
{
   ...
   if(*CmdLine) {
      if(ConvertValue(&CmdLine,&Bus)) {
         ...
         Ret = I2CRead(Bus, DevAddr, RegAddr, &RegData);
         if(Ret == RESULT_OK)
           NetPrintf("\r\nI2C Bus: %d Dev: 0x%02x Reg: 0x%02x Value: 0x%02x\r\n", Bus, DevAddr, RegAddr, RegData );
         else
           NetPrintf("\r\nI2C: Bus Error\r\n");
      }
   }
...
int InfoCmd(char *CmdLine)
{
   ...
   NetPrintf("Ethernet MAC (InUse): %02x:%02x:%02x:%02x:%02x:%02x\n",
      gOurMac[0], gOurMac[1], gOurMac[2], gOurMac[3], gOurMac[4], gOurMac[5] );

   NetPrintf("Ethernet MAC (Flash): %02x:%02x:%02x:%02x:%02x:%02x\n",
      gEtherMac[0], gEtherMac[1], gEtherMac[2], gEtherMac[3], gEtherMac[4], gEtherMac[5] );
...
int GPIOStatusCmd(char *CmdLine)
{
   ...
   NetPrintf("\r\nGPIO REGISTER = 0x%08x\r\n", REG_RD(GPIO_BASE + GPIO_INPUT) );
   return Ret;
}
...
int GPIODirCmd(char *CmdLine)
{
   ...
   if(*CmdLine) {
      if(ConvertValue(&CmdLine,&BitMask)) {
         Ret = RESULT_BAD_ARG;
      }
      else {
         REG_WR((GPIO_BASE + GPIO_DIRECTION), BitMask);
      }
   }
   else {
      NetPrintf("\r\nGPIO Direction = 0x%08x\r\n", REG_RD(GPIO_BASE + GPIO_DIRECTION) );
   }
...
int GPIOSetCmd(char *CmdLine)
{
   ...
   if(*CmdLine) {
      if(ConvertValue(&CmdLine,&BitMask)) {
         ...
         REG_WR((GPIO_BASE + GPIO_OUTPUT_SET), BitMask);
      }
   }
   else {
      Ret = RESULT_USAGE;
...
int GPIOClearCmd(char *CmdLine)
{
   ...
   if(*CmdLine) {
      if(ConvertValue(&CmdLine,&BitMask)) {
         ...
         REG_WR((GPIO_BASE + GPIO_OUTPUT_CLR), BitMask);
      }
   }
   else {
      Ret = RESULT_USAGE;
